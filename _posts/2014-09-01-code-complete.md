...
В описании правильной архитектуры сквозит ООП подход. Стандартные слова, что GUI должен быть GUI, масштабирование, безопасность и всё такое. Отличная копипаста в коммерческие предложения и тендеры. 

## Глава 4. Основные решения, которые приходится принимать при конструировании

Вот есть требования. И что идет дальше в книге под названием Совершенный код? Выбор языков программирования. Это смешно. Обосновнные специальным научным трудом и моделью оценки Cocomo II фразы в духе “если опыт с языком три года, эффективность выше на 30%, чем у тех, для кого язык новый” звучат хорошо только для руководителей. Стив наверняка выполнял какую-то задачу по минимальному объему текста, который корпоративные инженеры должны использовать в своих письмах руководителям и кадровикам. Особенно мне понравилась вот эта таблица:

![Глава 4, таблица](/img/cc/4.png)

Пример такой: если писал всю жизнь на Visual Basic (Fortran), то и на C++ будешь писать так же, перенеся туда всю свою быдлячью деревенскую сущность. Описание языков тоже довольно устаревшее, зато это та самая “старая школа”, хардкор. И снова на помощь приходит метафора: “одна часть картины в стиле импрессиониста, другая в классическом, третья в кубическом? — что получится в итоге”. А в итоге получится шедевр.

Какой софт купить, какую платформу выбрать и ещё подобных сто вопросов. Составить анкету и следовать ей — это корпоративный стиль. Представьте себе разработку операционной системы для x-box. Представьте себе программные комплексы в самолете. Ну и какой-нибудь Netflix. Разница в десятки людей с многолетним опытом инженерии, чтобы спроектировать систему — тогда может и пригодится этот список. 
...
Название части, два слова, которые не услышишь в реальной жизни, кроме как в резюме Senior Java Developer. 
...
Нужно уменшать сопряжение или связанность (не связность, это другое), которое бывает нескольких видов (через объект, параметры, семантичекое). И вот тут реально хорошие примеры приведены. 
...
Написанное часто хочется переписать заново (только не этот пост), чтобы стало лучше, чтобы стремиться к совершенству, это слово неслучайно присуствует в заголовке книги (complete). Тоже самое с проектированием — те же итерации. Можно идти сверху, но если сверху идти очень сложно и абстрактно для того чтобы уложить в ментальную модель, начинают снизу. Совместное проектирование за чашкой кофе или максимально формализованное. Если некому помочь — можно помочь самому себе, правой или левой рукой, как вы больше любите. Описываем, сохраняем всё и возвращаемся через неделю. Трезвый взгляд помогает, уверен Стив.
