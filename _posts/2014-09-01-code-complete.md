...
Наконец-то, господа. Ведь классы, это что-то связанное с кодом, да? Самое интересное начинается с самого абстрактного: абстрактных типов данных (Abstract data type). И когда клиентский код, который пользуется этим типом данных, не управляет, не контролирует многие из внутренних состояний и условий и поэтому код, связанный с АТД легко оптимизировать и расширять, значительно сократив вероятность breaking changes.

*Выражайте в интерфейсе класса согласованный уровень абстракции.* Мне нравится эта фраза, ей можно отвечать на любой вопрос человеку любого пола. 
